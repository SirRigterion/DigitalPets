...
import type {RegisterUser} from "@shared/types";
import {EmailNotVerifiedError} from "@shared/api/api.ts";
...
function PasswordStrength({ password }: { password: string }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      {score > 0 && (
        <p className={cn('text-xs font-body', score <= 1 ? 'text-red-400' : score <= 2 ? 'text-yellow-400' : score <= 3 ? 'text-blue-300' : 'text-green-400')}>
          {labels[score]}
        </p>
      )}
    </div>
...
export const RegisterPage = () => {
  ...
  const validate = () => {
    ...
    } else if (formData.user_login.trim().length < 3) {
      errors.username = 'Минимум 3 символа'
    } else if (!/^[a-zA-Z0-9_а-яА-Я]+$/.test(formData.user_login.trim())) {
    ...
    // ✅ ПАРОЛЬ (строгая проверка)
    const passwordError = validatePassword(formData.user_password)
    if (passwordError) {
    ...
    } else if (confirm !== formData.user_password) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register({
        user_login:     formData.user_login.trim(),
  ...
  return (
    <div className="min-h-screen bg-pet-bg flex flex-col items-center justify-center px-4 py-8">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none select-none">
        <div className="absolute -top-32 -right-32 w-96 h-96 bg-pet-teal/10 rounded-full blur-3xl animate-pulse-slow" />
        <div className="absolute -bottom-32 -left-32 w-96 h-96 bg-violet-500/10 rounded-full blur-3xl animate-pulse-slow" />
      </div>
      ...
      <div className="relative w-full max-w-sm animate-fadeIn">
        ...
        <div className="text-center mb-8">
          ...
          <p className="font-body text-white/40 text-sm mt-1">Создай аккаунт и заведи питомца!</p>
        </div>

        {/* Card */}
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur-sm space-y-4">
          <h2 className="font-display text-2xl text-white text-center">Регистрация</h2>

          {/* Success */}
          {success && (
            <div className="bg-green-500/15 border border-green-500/30 rounded-xl px-4 py-3 animate-pop text-center">
              <p className="text-green-400 text-sm font-body">🎉 Аккаунт создан! Заходим...</p>
            </div>
          )}

          {/* Global error */}
          {error && !success && (
            <div className="bg-red-500/15 border border-red-500/30 rounded-xl px-4 py-3 animate-pop">
              <p className="text-red-400 text-sm font-body text-center">⚠️ {error}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Username */}
            <div className="space-y-1.5">
              <label className="font-body text-xs font-bold text-white/50 uppercase tracking-wider">
                Логин
              </label>
              <input
                type="text"
                ...
                name="user_login"
                onChange={(e) => { handleChangeLogin(e); clearField('username') }}
                placeholder="CoolPetMaster"
                className={cn(
                  'w-full bg-white/8 border rounded-xl px-4 py-3 text-white font-body placeholder-white/25 focus:outline-none transition-all duration-200',
                  fieldErrors.username
                    ? 'border-red-500/60'
                    : 'border-white/15 focus:border-pet-teal/60 focus:bg-white/12'
                )}
              />
              {fieldErrors.username && (
                <p className="text-red-400 text-xs font-body">{fieldErrors.username}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                autoComplete="email"
                name={"user_email"}
                onChange={(e) => { handleChangeLogin(e);  clearField('email') }}
              ...
              />
              {fieldErrors.email && (
                <p className="text-red-400 text-xs font-body">{fieldErrors.email}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className={cn(
                    'w-full bg-white/8 border rounded-xl px-4 py-3 pr-12 text-white font-body placeholder-white/25 focus:outline-none transition-all duration-200',
              ...
              {fieldErrors.password && (
                <p className="text-red-400 text-xs font-body">{fieldErrors.password}</p>
              )}
              <PasswordStrength password={formData.user_password} />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className={cn(
                  ...
                  fieldErrors.confirm
                    ? 'border-red-500/60'
                    : confirm && confirm === formData.user_password
                    ? 'border-green-500/40'
                    : 'border-white/15 focus:border-pet-teal/60 focus:bg-white/12'
                )}
              />
              {fieldErrors.confirm && (
                <p className="text-red-400 text-xs font-body">{fieldErrors.confirm}</p>
              )}
              {!fieldErrors.confirm && confirm && confirm === formData.user_password && (
            ...
            <button
              ...
              className={cn(
                ...
                'bg-linear-to-r from-pet-teal to-violet-500',
                'hover:opacity-90 active:scale-95',
                'disabled:opacity-50 disabled:cursor-not-allowed',
                'shadow-lg shadow-pet-teal/20'
              )}
            >
              {isLoading ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-white/25 text-xs font-body">или</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>

          {/* Login link */}
          <p className="text-center font-body text-white/50 text-sm">
            Уже есть аккаунт?{' '}
            <Link
              to="/login"
              className="text-pet-glow font-bold hover:text-white transition-colors"
            >
              Войти
            </Link>
          </p>
