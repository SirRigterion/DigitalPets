...
export const LoginPage = () => {
  ...
  const [fieldErrors, setFieldErrors] = useState<{ user_identifier?: string; password?: string }>({})
  ...
  const validate = (data: LoginUser) => {
    ...
    if (!data.user_identifier.trim()) {
      errors.user_identifier = "Поле обязательно";
    } else {
      const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.user_identifier);
      const isUsername = /^[a-zA-Z0-9_]{3,}$/.test(data.user_identifier);

      if (!isEmail && !isUsername) {
        errors.user_identifier = "Введите email или логин (мин. 3 символа)";
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await login({ user_identifier: formData.user_identifier.trim(), password: formData.password })
      navigate('/pet')
    } catch (err) {
      // Если email не подтверждён — редиректим на страницу верификации
    ...
    <div className="min-h-screen bg-pet-bg flex flex-col items-center justify-center px-4 py-8">
      ...
      <div className="relative w-full max-w-sm animate-fadeIn">
        ...
        <div className="text-center mb-8">
          ...
          <p className="font-body text-white/40 text-sm mt-1">Войди, чтобы увидеть своего питомца</p>
        </div>

        {/* Card */}
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur-sm space-y-5">
          <h2 className="font-display text-2xl text-white text-center">Вход</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                autoComplete="email"
                onChange={(e) => { handleChangeLogin(e); setFieldErrors((p) => ({ ...p, email: undefined })) }}
                ...
                className={cn(
                  ...
                  fieldErrors.user_identifier
                    ? 'border-red-500/60 focus:border-red-500'
                    : 'border-white/15 focus:border-pet-glow/60 focus:bg-white/12'
                )}
              />
              {fieldErrors.user_identifier && (
                <p className="text-red-400 text-xs font-body">{fieldErrors.user_identifier}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) => { handleChangeLogin(e); setFieldErrors((p) => ({ ...p, password: undefined })) }}
                  placeholder="••••••••"
                  className={cn(
            ...
            <button
              ...
              className={cn(
                ...
                'bg-linear-to-r from-pet-glow to-pet-pink',
                'hover:opacity-90 active:scale-95',
                'disabled:opacity-50 disabled:cursor-not-allowed',
                'shadow-lg shadow-pet-glow/20'
              )}
            >
              {isLoading ? (
          ...
          <p className="text-center font-body text-white/50 text-sm">
            ...
            <Link
              to="/register"
              className="text-pet-teal font-bold hover:text-white transition-colors"
            >
              Зарегистрироваться
            </Link>
          </p>
